...
<div class="row">
    ...
    <div class="col-lg-4 col-12">
        <div class="small-box bg-secondary">
            <div class="inner">
                <h3>{{ $estadisticas['total_movimientos'] ?? 0 }}</h3>
...
<div class="card card-purple">
    ...
    <div class="card card-secondary">
        <div class="card-header">
            <h3 class="card-title"><i class="fas fa-history"></i> Historial de Acciones</h3>
            ...
            @if (isset($movimientos) && $movimientos->count() > 0)
                    ...
                    @foreach ($movimientosPorModulo as $modulo => $items)
                        <div class="col-md-2 col-sm-4 col-6 mb-2">
                            <div class="info-box bg-light mb-0">
                                <span class="info-box-icon bg-{{ $loop->index % 6 == 0 ? 'primary' : ($loop->index % 6 == 1 ? 'success' : ($loop->index % 6 == 2 ? 'warning' : ($loop->index % 6 == 3 ? 'danger' : ($loop->index % 6 == 4 ? 'info' : 'secondary')))) }}">
                                    <i class="fas fa-{{ $modulo == 'equipos' ? 'users' : ($modulo == 'cursos' ? 'graduation-cap' : ($modulo == 'reportes' ? 'fire' : ($modulo == 'emergencias' ? 'exclamation-triangle' : 'cog'))) }}"></i>
                                </span>
                                <div class="info-box-content">
                                    <span class="info-box-text">{{ ucfirst($modulo) }}</span>
                        ...
                        <tbody>
                            @foreach ($movimientos as $mov)
                                <tr>
                                    ...
                                    <td>
                                        <code class="small">{{ $mov->metodo_http }}</code>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                @if ($movimientos->count() >= 100)
                    <div class="alert alert-info mt-2">
                        ...
                        Mostrando los últimos 100 movimientos. Descarga el PDF para ver el historial completo.
                    </div>
                @endif
            @else
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> No tienes acciones registradas en el sistema.
                </div>
            @endif
        </div>
    </div>
